...
const float YAW = -87.5f; // Initialize yaw [flip screen to opposite of normal]
const float PITCH = 0.0f; // Initailize pitch
const float ROLL = 0.0f; // Initialize roll
const float SPEED = 4.0f; // Initialize speed
...
public:
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime)
	{
		...
		if (direction == UPPITCH) // If up pitch
			Pitch += 0.25f; // Increment pitch by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
		if (direction == DOWNPITCH) // If down pitch
			Pitch -= 0.25f; // Decrement pitch by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
		if (direction == UPYAW) // If up yaw
			Yaw += 0.25f; // Increment yaw by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
		if (direction == DOWNYAW) // If down yaw
			Yaw -= 0.25f; // Decrement yaw by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
		if (direction == UPROLL) // If up roll
			Roll += 0.25f; // Increment roll by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
		if (direction == DOWNROLL) // If down roll
			Roll -= 0.25f; // Decrement roll by 2 [degrees]
			updateCameraVectors(); // Update rotation using callback
	}
