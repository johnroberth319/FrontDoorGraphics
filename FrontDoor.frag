...
uniform vec3 frontDoorColor; // Receives frontDoorColor uniform
...
void main() {
    ...
    // specular
    float specularStrength = 0.01f; // Sets specularStrength
    vec3 viewDir = normalize(viewPos - FragPos); // Get viewDir
    vec3 reflectDir = reflect(-lightDir, norm); // Get reflectDir
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 2); // Get spec
    vec3 specular = specularStrength * spec * lightColor; // Set specular

    vec3 result = (ambient + diffuse + specular) * frontDoorColor; // Calculate result
